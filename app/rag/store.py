...
Day 4 先不接真实数据库，通过 Protocol 定义最小存储契约，
再用 `InMemoryDocumentStore` 做确定性测试；后续可替换为 PostgreSQL/pgvector。
"""
...
class DuplicateContentError(ValueError):
    """同一 content_hash 已存在时抛出。"""


class DuplicateIdError(ValueError):
    """同一 document id 已存在时抛出。"""


@runtime_checkable
class DocumentStore(Protocol):
    """存储层只暴露 ingestion 需要的最小能力。"""
