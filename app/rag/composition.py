"""RAG 运行期组合根（Week 16 Day 1）。

业务服务只依赖 `AsyncRetriever` 契约；这里负责选择具体实现：
- 稀疏分支：PostgreSQL 全文检索；
- 稠密分支：pgvector + 当前 embedder；
- 两者通过 RRF 融合成 hybrid retriever。

Embedding 是可替换边界。Week 16 先使用确定性的 `hash_embed` 作为基线，
接入真实 embedding API 时只替换 `dense_embedder`，业务服务不变。
"""
...
RagRetrieverFactory = Callable[[AsyncSession, str], AsyncRetriever]
